...
## The Git Log Command (and more commiting) 
- Untracked files get added with the `git add` command. 
- Modified files are files that are added, but have been changed again. 
- They all need to be added to staging to have the most updated version of them. 
    - Use `git status` to ensure that the most updated version of your files are in the next commit. 
- `git log` - returns a log of the commits for a given repository. 
- `git add .` - Used to stage all changes at once. 